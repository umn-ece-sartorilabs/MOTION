...
struct BenchmarkConfig {
    ...
    size_t repetitions;
    bool debug = false;
    int bits = 32;  // 32 or 64 bit integers
};
...
std::string OperationToString(OperationType op) {
    switch (op) {
        ...
        case OperationType::MATMUL: return "MatMul";
        case OperationType::CONV2D: return "Conv2D";
...
BenchmarkConfig ParseArguments(int argc, char** argv) {
    ...
    for (int i = 6; i < argc; ++i) {
        ...
        if (arg == "--debug" || arg == "-d") {
            ...
            if (i + 1 < argc) {
                config.bits = std::stoi(argv[++i]);
                if (config.bits != 32 && config.bits != 64) {
                    std::cerr << "Warning: bits must be 32 or 64, defaulting to 32" << std::endl;
                    config.bits = 32;
                }
            }
        } else {
...
std::vector<std::uint32_t> RandomUnsigned32(size_t size, size_t party_id, uint32_t min_val, uint32_t max_val) {
    uint32_t seed = 1000 + party_id * 12345;
    std::mt19937 gen(seed);
    std::uniform_int_distribution<std::uint32_t> dis(min_val, max_val);
    std::vector<std::uint32_t> v(size);
    for (size_t i = 0; i < size; ++i) v[i] = dis(gen);
    return v;
}

std::vector<std::uint64_t> RandomUnsigned64(size_t size, size_t party_id, uint64_t min_val, uint64_t max_val) {
    uint32_t seed = 1000 + party_id * 12345;
    std::mt19937 gen(seed);
    std::uniform_int_distribution<std::uint64_t> dis(min_val, max_val);
...
RunTimeStatistics RunMatMulBenchmark(PartyPointer& party, size_t dim, bool debug) {
    ...
    if (debug) std::cout << "[DEBUG] MatMul Dimension: " << dim << "x" << dim << " (" << matrix_elements << " elements)\n";

    // Generate Inputs (Dim x Dim matrices flattened)
    auto mat_a = RandomUnsigned(matrix_elements, my_id, 0, 10);
    
    // Sharing: P0 inputs A, P1 inputs B.
    // We use the generated 'mat_a' as the input source for whichever party is providing input.
    // If I am P0, I provide my 'mat_a' as A. If I am P1, I provide my 'mat_a' as B.
    // The other party provides dummy zeros for that share.
    
    auto dummy = std::vector<uint8_t>(matrix_elements, 0);
    
    // Share A (input from P0)
    ShareWrapper share_a = party->In<MpcProtocol::kArithmeticGmw>((my_id == 0) ? mat_a : dummy, 0);
    
    // Share B (input from P1)
    ShareWrapper share_b = party->In<MpcProtocol::kArithmeticGmw>((my_id == 1) ? mat_a : dummy, 1);
    
    // Naive O(N^3) Matrix Multiplication on Shares
...
RunTimeStatistics RunConv2DBenchmark(PartyPointer& party, size_t dim, bool debug) {
    ...
    if (debug) std::cout << "[DEBUG] Conv2D Image: " << dim << "x" << dim << " Kernel: 3x3 Output: " << out_dim << "x" << out_dim << "\n";
    ...
    // INPUT SHARES: P0 inputs Image, P1 inputs Kernel
    ShareWrapper share_img = party->In<MpcProtocol::kArithmeticGmw>((my_id == 0) ? my_img : dummy_img, 0);
    ShareWrapper share_kernel = party->In<MpcProtocol::kArithmeticGmw>((my_id == 1) ? my_kernel : dummy_kernel, 1);
    ...
    for (size_t r = 0; r < out_dim; ++r) {
        for (size_t c = 0; c < out_dim; ++c) {
             // 3x3 dot product
             ShareWrapper sum = img_vec[(r)*dim + c] * kern_vec[0]; // partial init loop unrolling usually needed
             // Actually, initialize with 0? Or first element?
             // Lets accumulate.
             // We need a 'Zero' share or just start with first mult.
             
             // First element [0,0]
             sum = img_vec[(r+0)*dim + (c+0)] * kern_vec[0*kernel_dim+0];
             
             for (size_t ki = 0; ki < kernel_dim; ++ki) {
                 for (size_t kj = 0; kj < kernel_dim; ++kj) {
                     if (ki==0 && kj==0) continue; // already done
