...
enum class OperationType { SUM, COUNT, RELU, BILLIONAIRE, MATMUL, CONV2D };
...
OperationType ParseOperation(const std::string& op_str) {
  ...
  if (s == "matmul") return OperationType::MATMUL;
  if (s == "conv2d") return OperationType::CONV2D;
  throw std::runtime_error("Invalid operation. Use: sum, count, relu, billionaire, matmul, conv2d");
}
...
BenchmarkConfig ParseArguments(int argc, char** argv) {
  if (argc < 6) {
    std::cerr << "Usage: " << argv[0]
              << " <my-id> <party0-info> <party1-info> <operation> <vector-size> [repetitions] "
                 "[--debug|-d]\n";
...
std::vector<std::uint8_t> RandomUnsigned(size_t size, size_t party_id, uint8_t min_val,
                                         uint8_t max_val) {
...
std::vector<std::int8_t> RandomSigned(size_t size, size_t party_id, int8_t min_val,
                                      int8_t max_val) {
  uint32_t seed = 1000 + party_id * 12345;
  std::mt19937 gen(seed);
...
std::unique_ptr<communication::CommunicationLayer> SetupCommunication(
  ...
  for (int attempt = 1; attempt <= kMaxRetries; ++attempt) {
    try {
      ...
      comm_layer = std::make_unique<communication::CommunicationLayer>(config.my_id,
                                                                       helper.SetupConnections());
...
RunTimeStatistics RunSumBenchmark(PartyPointer& party, size_t vector_size, bool debug) {
  ...
  if (debug) {
    ...
    std::cout << std::endl;
    std::cout << "[DEBUG] Party " << my_party_id << " expected local sum = " << expected_local_sum
              << std::endl;

    uint64_t verify_sum = 0;
    for (size_t i = 0; i < vector_size; ++i) verify_sum += my_local_input[i];
    std::cout << "[DEBUG] Party " << my_party_id << " verified total local sum = " << verify_sum
              << std::endl;
  }
  ...
  if (debug) {
    std::cout << "[DEBUG] Reconstruction completed. Result vector size: " << result.size()
              << std::endl;
    if (!result.empty()) {
      std::cout << "[DEBUG] Raw reconstructed value: " << static_cast<int>(result[0]) << std::endl;
      std::cout << "[DEBUG] Party " << my_party_id << " local sum: " << expected_local_sum
                << std::endl;
      std::cout << "[DEBUG] MPC result (sum of both parties): " << static_cast<int>(result[0])
                << std::endl;
    }
...
RunTimeStatistics RunCountBenchmark(PartyPointer& party, size_t vector_size, bool debug) {
  ...
  for (auto v : my_local_input)
    if (v > 50) local_clear_count++;

  std::cout << "Party " << my_id << ": COUNT elements > 50 from " << vector_size
            << " integers (0-100)" << std::endl;
  ...
  ShareWrapper arithmetic_one =
      party->In<MpcProtocol::kArithmeticGmw>(std::vector<std::uint8_t>{1}, 0);
...
RunTimeStatistics RunReLUBenchmark(PartyPointer& party, size_t vector_size, bool debug) {
  ...
  std::cout << "Party " << my_id << ": ReLU max(0,x) on " << vector_size
            << " signed integers (-50 to +50)" << std::endl;
  ...
  // CORRECT INPUT SHARING
  auto input_p0 =
      (my_id == 0) ? my_local_input_unsigned : std::vector<std::uint8_t>(vector_size, 0);
  auto input_p1 =
      (my_id == 1) ? my_local_input_unsigned : std::vector<std::uint8_t>(vector_size, 0);
  ShareWrapper share_p0 = party->In<MpcProtocol::kArithmeticGmw>(input_p0, 0);
  ShareWrapper share_p1 = party->In<MpcProtocol::kArithmeticGmw>(input_p1, 1);
  ShareWrapper combined_inputs = share_p0 + share_p1;
  ...
  party->Run();
  auto result = out.As<std::vector<std::uint8_t>>();
  // The result is a sum of unsigned values, which needs to be cast back to signed for
  // interpretation
...
RunTimeStatistics RunBillionaireBenchmark(PartyPointer& party, size_t vector_size, bool debug) {
  ...
  std::cout << "Party " << my_id << ": BILLIONAIRE wealth comparison on " << vector_size
            << " values" << std::endl;

  // 2. Share all six input vectors correctly
  // Party 0's shares
  ShareWrapper p0_cash_share =
      party->In<MpcProtocol::kArithmeticGmw>((my_id == 0) ? my_cash : dummy_vec, 0);
  ShareWrapper p0_prop_share =
      party->In<MpcProtocol::kArithmeticGmw>((my_id == 0) ? my_prop : dummy_vec, 0);
  ShareWrapper p0_stock_share =
      party->In<MpcProtocol::kArithmeticGmw>((my_id == 0) ? my_stock : dummy_vec, 0);

  // Party 1's shares
  ShareWrapper p1_cash_share =
      party->In<MpcProtocol::kArithmeticGmw>((my_id == 1) ? my_cash : dummy_vec, 1);
  ShareWrapper p1_prop_share =
      party->In<MpcProtocol::kArithmeticGmw>((my_id == 1) ? my_prop : dummy_vec, 1);
  ShareWrapper p1_stock_share =
      party->In<MpcProtocol::kArithmeticGmw>((my_id == 1) ? my_stock : dummy_vec, 1);
  ...
  ShareWrapper arithmetic_one =
      party->In<MpcProtocol::kArithmeticGmw>(std::vector<std::uint8_t>(vector_size, 1), 0);
  ...
  party->Run();
  auto result = out.As<std::vector<std::uint8_t>>();
  std::cout << "Billionaire total comparisons (P0 wins + P1 wins) = " << static_cast<int>(result[0])
            << std::endl;
...
RunTimeStatistics RunMatMulBenchmark(PartyPointer& party, size_t dim, bool debug) {
  auto start = std::chrono::high_resolution_clock::now();
  size_t my_id = party->GetConfiguration()->GetMyId();
  size_t matrix_elements = dim * dim;

  if (debug)
    std::cout << "[DEBUG] MatMul Dimension: " << dim << "x" << dim << " (" << matrix_elements
              << " elements)\n";

  // Generate Inputs (Dim x Dim matrices flattened)
  auto mat_a = RandomUnsigned(matrix_elements, my_id, 0, 10);
  auto dummy = std::vector<uint8_t>(matrix_elements, 0);

  // Share A (input from P0), Share B (input from P1)
  ShareWrapper share_a = party->In<MpcProtocol::kArithmeticGmw>((my_id == 0) ? mat_a : dummy, 0);
  ShareWrapper share_b = party->In<MpcProtocol::kArithmeticGmw>((my_id == 1) ? mat_a : dummy, 1);

  // Naive O(N^3) Matrix Multiplication: C[i][j] = Sum(A[i][k] * B[k][j])
  auto a_vec = share_a.Unsimdify();
  auto b_vec = share_b.Unsimdify();
  std::vector<ShareWrapper> c_vec;
  c_vec.reserve(matrix_elements);

  for (size_t i = 0; i < dim; ++i) {
    for (size_t j = 0; j < dim; ++j) {
      ShareWrapper sum = a_vec[i * dim + 0] * b_vec[0 * dim + j];
      for (size_t k = 1; k < dim; ++k) {
        sum = sum + (a_vec[i * dim + k] * b_vec[k * dim + j]);
      }
      c_vec.push_back(sum);
    }
  }

  ShareWrapper c_matrix = ShareWrapper::Simdify(c_vec);
  auto out = c_matrix.Out();

  party->Run();
  auto result = out.As<std::vector<uint8_t>>();

  if (debug) std::cout << "[DEBUG] MatMul[0][0] result: " << (int)result[0] << "\n";

  party->Finish();
  auto end = std::chrono::high_resolution_clock::now();
  std::cout << "MatMul execution time: "
            << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << " ms"
            << std::endl;
  return party->GetBackend()->GetRunTimeStatistics().front();
}

RunTimeStatistics RunConv2DBenchmark(PartyPointer& party, size_t dim, bool debug) {
  auto start = std::chrono::high_resolution_clock::now();
  size_t my_id = party->GetConfiguration()->GetMyId();
  size_t img_size = dim * dim;
  size_t kernel_dim = 3;
  size_t kernel_size = kernel_dim * kernel_dim;
  size_t out_dim = dim - kernel_dim + 1;
  size_t out_size = out_dim * out_dim;

  if (debug)
    std::cout << "[DEBUG] Conv2D Image: " << dim << "x" << dim << " Kernel: 3x3 Output: " << out_dim
              << "x" << out_dim << "\n";

  // Generate Inputs
  auto my_img = RandomUnsigned(img_size, my_id, 0, 10);
  auto my_kernel = RandomUnsigned(kernel_size, my_id, 0, 5);
  auto dummy_img = std::vector<uint8_t>(img_size, 0);
  auto dummy_kernel = std::vector<uint8_t>(kernel_size, 0);

  // P0 inputs Image, P1 inputs Kernel
  ShareWrapper share_img =
      party->In<MpcProtocol::kArithmeticGmw>((my_id == 0) ? my_img : dummy_img, 0);
  ShareWrapper share_kernel =
      party->In<MpcProtocol::kArithmeticGmw>((my_id == 1) ? my_kernel : dummy_kernel, 1);

  auto img_vec = share_img.Unsimdify();
  auto kern_vec = share_kernel.Unsimdify();

  std::vector<ShareWrapper> out_vec;
  out_vec.reserve(out_size);

  for (size_t r = 0; r < out_dim; ++r) {
    for (size_t c = 0; c < out_dim; ++c) {
      // First element [0,0]
      ShareWrapper sum = img_vec[(r + 0) * dim + (c + 0)] * kern_vec[0 * kernel_dim + 0];

      for (size_t ki = 0; ki < kernel_dim; ++ki) {
        for (size_t kj = 0; kj < kernel_dim; ++kj) {
          if (ki == 0 && kj == 0) continue;
          sum = sum + (img_vec[(r + ki) * dim + (c + kj)] * kern_vec[ki * kernel_dim + kj]);
        }
      }
  ...
  ShareWrapper out_matrix = ShareWrapper::Simdify(out_vec);
  auto out = out_matrix.Out();

  party->Run();
  auto result = out.As<std::vector<uint8_t>>();

  if (debug) std::cout << "[DEBUG] Conv2D[0][0]: " << (int)result[0] << "\n";

  party->Finish();
  auto end = std::chrono::high_resolution_clock::now();
  std::cout << "Conv2D execution time: "
            << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << " ms"
            << std::endl;
  return party->GetBackend()->GetRunTimeStatistics().front();
}
...
int main(int argc, char** argv) {
  try {
    ...
    std::cout << "=== MOTION " << OperationToString(config.operation)
              ...
              << "\nOperation: " << OperationToString(config.operation)
    ...
    for (size_t rep = 0; rep < config.repetitions; ++rep) {
      ...
      switch (config.operation) {
        ...
        case OperationType::BILLIONAIRE:
          ...
          break;
        case OperationType::MATMUL:
          stats = RunMatMulBenchmark(party, config.vector_size, config.debug);
          break;
        case OperationType::CONV2D:
          stats = RunConv2DBenchmark(party, config.vector_size, config.debug);
          break;
      }
